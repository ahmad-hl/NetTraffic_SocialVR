...
p1 = axes[0].bar(ind, rttMeans, width, yerr=rttCIs, color='dodgerblue', log=False,
            capsize=3, error_kw=dict(elinewidth=1, ecolor='blue'), label='Network RTT')
p2 = axes[1].bar(ind, proc_rttMeans, width, yerr=proc_rttCIs, color='skyblue', log=False,
            capsize=3, error_kw=dict(elinewidth=1, ecolor='black'), label='Application RTT')
plt.margins(0.01, 0)
...
axes[0].set_ylabel('Application RTT (ms)', fontsize=14, labelpad=-1)

axes[1].spines['right'].set_visible(False)
axes[1].spines['top'].set_visible(False)
axes[1].tick_params(axis='both', which='major', labelsize=15)
axes[1].tick_params(axis='both', which='minor', labelsize=12)
axes[1].set_ylabel('Network RTT (ms)', fontsize=14, labelpad=-1)
