...
in_dir_names = ['results_dthub/synth8Oculus','results_dthub/synth9Oculus','results_dthub/synth10Oculus']
...
for in_dir_name in in_dir_names:
    file_name = '../{}/avroom_OculusLoad_time.csv'.format(in_dir_name)
...
for load, av in zip(loadMeans,avatarMeans):
    ...
    ax.text(i, av+4, "{0:.1f}".format(av), color='gray', fontsize=16)
    i = i + 1
...
plt.legend(loc='lower center', fontsize=13, title="Latency", fancybox=True, ncol=2)
parentDir = os.path.dirname(os.path.realpath(__file__))
Fig_PATH = os.path.join(parentDir, 'figures', 'oculusLoad_latency.pdf')
